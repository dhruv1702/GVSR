...
class SmoothenDict:
    ...
    def smooth_all(self):
        all_smooth = 0
        for k in self.keys:
            all_smooth+=self.smooth_vals[k].smooth
        return all_smooth
...
class Learner:
    ...
    def train_epoch(self, mb) -> List[torch.tensor]:
        ...
        for batch_id, batch in enumerate(progress_bar(self.data.train_dl, parent=mb)):
            ...
            out = self.mdl(batch)
            out_loss = self.loss_fn(out)
            loss = sum(out_loss.values())
            loss = loss.mean()
    ...
    def fit(self, epochs: int, lr: float, params_opt_dict: Optional[Dict] = None):
        ...
        try:
            ...
            for epoch in mb:
                ...
                met_to_use = valid_acc[self.met_keys[0]].cpu() #cider
    ...
    def testing(self, db: Dict[str, DataLoader]):
        ...
        for dl_name, dl in tqdm(db.items(), total=len(db)):
            out_loss, out_acc = self.validate(dl)
            # print('Iou_raw: {}, Iou@30: {}, Iou@50: {}'.format(ious[0], ious[1], ious[2]))
            time.sleep(2)
