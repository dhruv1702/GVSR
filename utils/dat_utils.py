...
def pad_tokens(
    ...
    if append_eos:
        if curr_len + 1 >= max_len:
            sent_out_enc[0] = bos_index
            sent_out_enc[1:max_len] = lst[:max_len-1]
            sent_out_enc[max_len - 1] = eos_index
            out_len = max_len
        else:
            sent_out_enc[0] = bos_index
            if pad_side == "right":
                sent_out_enc[1:curr_len+1] = lst
            else:
                sent_out_enc[-curr_len+1:] = lst
            sent_out_enc[curr_len+1] = eos_index
            out_len = curr_len + 2
    else:
        if curr_len >= max_len:
